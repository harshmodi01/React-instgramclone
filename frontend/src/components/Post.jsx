...
const Post = ({ post }) => {
  ...
  const addtofav = async () => {
    ...
    try {
      const res = await axios.get(`http://localhost:3000/api/v1/post/${post?._id}/addtofav`, { withCredentials: true });
      if (res.data.success) {
        toast.success(res.data.message);
      }
    } catch (error) {
  ...
  return (
    <div className="my-8 w-full max-w-sm mx-auto">
      <div className="flex items-center justify-between">
        ...
        {/* <Button onClick={addtofav} variant='ghost' className="cursor-pointer w-fit">Add to favorites</Button> */}
        {/* {
      ...
      <div className="flex items-center justify-between my-2">
        <div className="flex items-center gap-3">
          ...
          <FiStar onClick={addtofav} size={'24'} className="cursor-pointer hover:text-gray-600" />
        </div>
